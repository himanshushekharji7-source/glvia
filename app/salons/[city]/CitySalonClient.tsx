...
interface CitySalonClientProps {
  cityName: string;
  citySlug: string;
  salons: Salon[];
  nearbyAreas: string[];
...
const filters = ["All", "Top Rated", "Budget", "Premium", "Nearby"];

export default function CitySalonClient({ cityName, citySlug, salons, nearbyAreas }: CitySalonClientProps) {
  ...
  const filteredSalons = useMemo(() => {
    ...
    return list;
  }, [salons, searchQuery, activeFilter]);

  return (
    <div className="min-h-dvh bg-slate-50 pb-nav">
      {/* ─── Hero Header ─── */}
      <div className="relative bg-gradient-to-br from-pink-600 via-rose-500 to-purple-600 px-5 pt-5 pb-8">
        {/* Back button */}
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-white/80 text-[12px] font-medium">
              {salons.length} premium salons • Uttar Pradesh
            </p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={`Search salons in ${cityName}...`}
      ...
      {nearbyAreas.length > 0 && (
        <div className="px-5 pb-3 overflow-x-auto no-scrollbar flex gap-2">
          {nearbyAreas.map((area) => (
            <span
              key={area}
              className="px-3 py-1 rounded-lg text-[11px] font-semibold bg-purple-50 text-purple-700 border border-purple-100 whitespace-nowrap"
            >
              📍 {area}
            </span>
          ))}
      ...
      <div className="px-5 pb-2">
        <p className="text-[13px] text-slate-500">
          <span className="font-bold text-slate-800">{filteredSalons.length}</span> salons found
          in {cityName}
        </p>
      </div>
        ...
        {filteredSalons.length > 0 ? (
          filteredSalons.map((salon, i) => (
            <Link
              href={`/salon/${salon.id}`}
              key={salon.id}
              className="block border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:border-pink-200 hover:shadow-md transition-all cursor-pointer bg-white animate-fadeInUp"
            ...
            >
              <div className="w-full h-[160px] relative">
                <Image
                  ...
                  alt={`${salon.name} - Premium Salon in ${cityName}, Uttar Pradesh`}
                  fill
                ...
                {salon.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {salon.tags.slice(0, 4).map((tag) => (
                      <span
                        key={tag}
                        className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-pink-50 text-pink-600"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center bg-white border border-slate-100 rounded-3xl shadow-sm px-6">
            ...
            <h3 className="text-lg font-extrabold text-slate-800 mb-1">
              No salons found in {cityName}
            </h3>
            <p className="text-[13px] text-slate-500 max-w-[280px] leading-relaxed mb-5">
              We&apos;re expanding to {cityName} soon! Meanwhile, explore salons in nearby cities.
            </p>
            <Link
              href="/at-the-salon"
