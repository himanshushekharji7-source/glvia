...
function UnifiedLoginContent() {
  ...
  // Auto-focus OTP input when view shifts to OTP screen
  useEffect(() => {
    if (view === "otp" && otpInputRef.current) {
  ...
  const handleOtpChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setOtpCode(val);
    // Auto-submit when exactly 6 digits are fully filled
    if (val.length === 6) {
      verifyCode(val);
    }
  };
  ...
  return (
    <div className="relative min-h-dvh flex items-center justify-center bg-[#f8f9fa] font-sans antialiased py-8 px-4 overflow-hidden">
      <div id="recaptcha-container"></div>
      
      {/* Premium background soft glow blobs */}
      <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-pink-500/5 blur-[100px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full bg-purple-500/5 blur-[100px] pointer-events-none" />

      {/* ======================= ROLE SELECTION ======================= */}
      {view === "role-selection" && (
        <div className="relative w-full max-w-[440px] bg-white rounded-[32px] border border-slate-100 p-8 sm:p-10 shadow-[0_16px_40px_rgba(0,0,0,0.03)] text-center transition-all duration-300">
          <h1 className="text-4xl font-extrabold bg-gradient-to-r from-[#e11d48] to-[#9333ea] bg-clip-text text-transparent mb-3.5 tracking-tight">
            Glvia.com
          </h1>
          ...
          <div className="space-y-4">
            <button 
              ...
              className="w-full group bg-white border border-slate-100 p-4.5 rounded-2xl flex items-center gap-4 hover:border-pink-200 hover:shadow-[0_12px_24px_rgba(236,72,153,0.06)] transition-all duration-300 text-left cursor-pointer"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-400 font-semibold leading-tight">Grow your salon business with Glvia</p>
              </div>
              <span className="material-icons-round text-slate-300 group-hover:text-[#e11d48] group-hover:translate-x-1 transition-all duration-300 text-[18px]">arrow_forward_ios</span>
            </button>

            <button 
              onClick={() => { setPersona("customer"); setView("login"); }}
              className="w-full group bg-white border border-slate-100 p-4.5 rounded-2xl flex items-center gap-4 hover:border-purple-200 hover:shadow-[0_12px_24px_rgba(124,58,237,0.06)] transition-all duration-300 text-left cursor-pointer"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-400 font-semibold leading-tight">Book premium beauty services near you</p>
              </div>
              <span className="material-icons-round text-slate-300 group-hover:text-[#7c3aed] group-hover:translate-x-1 transition-all duration-300 text-[18px]">arrow_forward_ios</span>
            </button>
          </div>
        </div>
      )}
      ...
      {/* ======================= LOGIN VIEW ======================= */}
      {view === "login" && (
        <div className="relative w-full max-w-[440px] bg-white rounded-[32px] border border-slate-100 p-8 sm:p-10 shadow-[0_16px_40px_rgba(0,0,0,0.03)] transition-all duration-300">
          
          {/* Back Button */}
          <button 
            type="button"
            onClick={() => setView("role-selection")}
            className="absolute top-6 left-6 text-slate-400 hover:text-slate-900 font-bold text-[11px] uppercase tracking-wider flex items-center gap-1 transition-colors cursor-pointer"
          >
          ...
          {isLoading && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-20 flex flex-col items-center justify-center rounded-[32px]">
              ...
              <span className="text-xs font-bold text-slate-800">Connecting...</span>
            </div>
          )}
          ...
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div className="relative group flex items-center border border-slate-200 rounded-2xl bg-white shadow-sm focus-within:border-[#e11d48]/50 focus-within:ring-4 focus-within:ring-pink-500/5 transition-all overflow-hidden px-4">
              <span className="material-icons-round text-[18px] text-slate-400">phone_iphone</span>
              <span className="text-[14px] font-bold text-slate-900 ml-2.5 shrink-0">+91</span>
              <input 
                type="tel" 
                ...
                required 
                className="w-full pl-2 pr-4 py-4 bg-transparent text-[14px] font-bold text-slate-900 placeholder-slate-400 focus:outline-none" 
              />
            </div>
            
            <button 
              type="submit" 
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#e11d48] to-[#9333ea] text-white text-xs font-bold shadow-md hover:opacity-95 transition-opacity flex items-center justify-center gap-1.5 cursor-pointer"
            >
          ...
          <button 
            ...
            className="w-full py-3.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300 rounded-2xl font-bold text-xs transition-all flex items-center justify-center gap-2.5 shadow-sm cursor-pointer"
          >
      ...
      {view === "otp" && (
        <div className="relative w-full max-w-[440px] bg-white rounded-[32px] border border-slate-100 p-8 sm:p-10 shadow-[0_16px_40px_rgba(0,0,0,0.03)] transition-all duration-300">
          ...
          {isLoading && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-20 flex flex-col items-center justify-center rounded-[32px]">
              <div className="w-8 h-8 border-3 border-slate-100 border-t-[#e11d48] rounded-full animate-spin mb-3" />
              <span className="text-xs font-bold text-slate-800">Verifying code...</span>
            </div>
          )}
          ...
          <div className="text-center mt-6 mb-8">
            ...
            <p className="text-[12px] text-slate-400 font-semibold px-4 leading-relaxed">
              ...
              <button 
                type="button" 
                onClick={() => setView("login")} 
                className="ml-2 text-xs font-bold text-[#e11d48] hover:underline cursor-pointer"
              >
                Change
              </button>
            </p>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-6">
            <div className="relative group">
              <input 
                ...
                required 
                className="w-full text-center tracking-[0.8em] pl-[0.8em] text-[28px] font-black py-3.5 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-200 focus:outline-none focus:border-[#e11d48]/50 focus:ring-4 focus:ring-pink-500/5 transition-all" 
              />
            </div>
            ...
            <button 
              ...
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#e11d48] to-[#9333ea] text-white text-xs font-bold shadow-md hover:opacity-95 transition-opacity disabled:opacity-50 flex items-center justify-center cursor-pointer"
            >
              Verify Code
            </button>
      ...
      {/* ======================= SUCCESS VIEW ======================= */}
      {view === "success" && (
        <div className="relative w-full max-w-[400px] bg-white rounded-[32px] border border-slate-100 p-8 sm:p-10 shadow-[0_16px_40px_rgba(0,0,0,0.03)] text-center transition-all duration-300">
          <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center border border-emerald-100">
            <span className="material-icons-round text-[36px] font-bold">check</span>
          </div>
          ...
          <button 
            ...
            className="w-full py-4 rounded-2xl bg-slate-900 text-white text-xs font-bold shadow-md hover:bg-slate-800 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
          >
